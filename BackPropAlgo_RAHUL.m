function[V W]= BackPropAlgo_RAHUL(Input, Output)
...
delta_W=zeros(size(W));  % ADDED BY RAHUL
delta_V=zeros(size(V));  % ADDED BY RAHUL
...
%Created By : RAHUL SINGH SAHRAWAT
...
%result(q)= Output_of_OutputLayer % Added by rahul
%Now we need to calculate the Error using Root Mean Square method
q=q+1;   % added by rahul 
